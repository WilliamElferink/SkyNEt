...
class GenePool(object):
    ...
    def __init__(self, config_obj, dataset):
    ...
    def AddNoise(self):
        ...
        for i in range(0,self.genes):
            self.newpool[sum(self.partition[:1]):sum(self.partition[:2]), i] = self.pool[:self.partition[1],i] + 0.02 * np.random.uniform(self.generange[i][0], self.generange[i][1], size=(self.partition[1],))
