...
filepath = r'D:\\data\\Tao\\191204-B-Nsub-2min-another-50nm\\BiasSensewANS100Hz\\'
...
for vbias in Input:
	...
	print(vbias)
	InstrumentImporter.IVVIrack.setControlVoltages(ivvi, np.array([vbias]))
	x = np.zeros([2,siglen*fs])
	x[0] = 0.01*np.sin(2*np.pi*ii*np.arange(siglen*fs)/fs)
