...
filepath = r'D:\\data\\Tao\\TCTOtest\\TCTO_A_HBPS_oxideremoved\\'		
name = 'bg1bg2anothertime.txt'
instrument = 0  #choose between nidaq (1) and adwin (0)
InstrumentImporter.reset(0,0, exit=False)
InstrumentImporter.IVVIrack.setControlVoltages(ivvi, np.array([2000]))
...
Input = np.ones((2,len(Input1)+len(Input2)+len(Input3)))
...
plt.figure()
plt.plot(Input[0,0:len(output)]*Vgain, output)
plt.show()

datetime = time.strftime("%d_%m_%Y_%H%M%S")
filepath = filepath + '\\' + datetime + '_' + name
np.savetxt(filepath, (Input[0,:]*Vgain,output))
